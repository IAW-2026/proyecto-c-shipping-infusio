...
export default function LogisticsPage() {
    ...
    useEffect(() => {
        window.localStorage.setItem(
            TRACKINGS_STORAGE_KEY,
            JSON.stringify({ trackings, assignments }),
        )
                    ...
                    <div className="max-w-2xl">
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:min-w-105">
                        <div className="rounded-2xl border border-border bg-background p-4">
                            <p className="text-xs uppercase tracking-widest text-muted-foreground">Pendientes</p>
                            <p className="mt-2 text-2xl font-semibold text-foreground">{pendingShipments.length}</p>
                        </div>
                        <div className="rounded-2xl border border-border bg-background p-4">
                            <p className="text-xs uppercase tracking-widest text-muted-foreground">Asignados</p>
                            <p className="mt-2 text-2xl font-semibold text-foreground">{assignmentCount}</p>
                        </div>
                        <div className="rounded-2xl border border-border bg-background p-4">
                            <p className="text-xs uppercase tracking-widest text-muted-foreground">En reparto</p>
                            <p className="mt-2 text-2xl font-semibold text-foreground">
                                {pendingShipments.filter((shipment) => shipment.latestStatus.toLowerCase().includes("reparto")).length}
                            </p>
                        </div>
                        <div className="rounded-2xl border border-border bg-background p-4">
                            <p className="text-xs uppercase tracking-widest text-muted-foreground">Entregados</p>
                            <p className="mt-2 text-2xl font-semibold text-foreground">{deliveredShipments.length}</p>
                        </div>
                    </div>
                </div>
            </section>
                                    ...
                                    <div className="flex flex-wrap items-start justify-between gap-4">
                                        ...
                                        <div className="rounded-2xl border border-border px-4 py-3">
                                            ...
                                            <p className="mt-1 text-xs text-muted-foreground">
                                                {new Date(selectedShipment.latestDatetime).toLocaleString("es-AR")}
                                            </p>
                                        </div>
                                    </div>
                                            ...
                                            deliveredShipments.map((shipment) => (
                                                <article key={shipment.id} className="rounded-xl border border-border bg-background p-4">
                                                    ...
                                                    <p className="mt-3 text-xs text-muted-foreground">
                                                        Última actualización: {new Date(shipment.latestDatetime).toLocaleString("es-AR")}
                                                    </p>
                                                </article>
                                            ))
                                        )}
                            ...
                            {recentlyUpdatedTrackings.map((tracking) => (
                                <article key={tracking.id} className="rounded-xl border border-border bg-background p-4">
                                    <div className="flex items-start justify-between gap-3">
                                        ...
                                        <span className="text-xs text-muted-foreground">{new Date(tracking.datetime).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}</span>
                                    </div>
